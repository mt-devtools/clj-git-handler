...
(defn- read-submodule!
  ...
  ; 3. If a previously detected submodule is represented in the 'deps.edn' file as a dependency,
  ;    that represented submodule will be declared as an inner dependency and will be stored
  ;    in the 'INNER-DEPENDENCIES' atom.
  ;
  ; @param (string) submodule-path
  ; @param (map) submodule-props
  ; {:git-url (string)}
  [submodule-path {:keys [git-url]}]
  (if-let [{:keys [deps]} (core.env/read-submodule-deps-edn submodule-path)]
